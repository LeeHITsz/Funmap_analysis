...
for(Pos in ld_list){
    ...
    # trait = 'TG'
        ...
        CARMA.results<-CARMA(z.list,ld.list,lambda.list=lambda.list,outlier.switch=F,rho.index=0.95,Max.Model.Dim=2000,all.iter=4,all.inner.iter=4)
        
        result = data.frame(
            "SNPs" = sumstat[,1],
            "PIP" = CARMA.results[[1]]$PIPs
        )
        fwrite(x = result, file = paste('chr',Chr,'/CARMA_result/','chr',Chr,'_',Pos*1000000+1,'_',(Pos+3)*1000000+1,'_',trait,'.csv', sep=""), sep = ",", quote = F, na = "NA", row.names = F, col.names = F)
        ...
        fwrite(x = sets, file = paste('chr',Chr,'/CARMA_sets/','chr',Chr,'_',Pos*1000000+1,'_',(Pos+3)*1000000+1,'_',trait,'.sets', sep=""), sep = ",", quote = F, na = "NA", row.names = F, col.names = F)
